<?php 
  session_start();
   if(!isset($_SESSION['mail_verify']) || !isset($_SESSION['mail_access'])){
  ...
  $message="";
   if(isset($_POST['check-code'])){ 
      ...
      if($num == 1){
          ...
          $update = "UPDATE user_table SET code = '$code', statut = '$statut' WHERE code = '$conf_code' AND email = '$email'";
           $query = mysqli_query($con, $update);
           if($query){
                   $info="Votre adresse e-mail à bien été validé, vous pouvez connnecter à votre compte.";
                   $_SESSION['info_mail'] = $info;
                   unset($_SESSION['mail_access']);
                   header('location: ../login.php');
           }
           else{
            $message = "<div class=\"d-flex justify-content-center\">
        ...
        <div class=\"alert alert-success alert-dismissible fade show text-center pr-2\" role=\"alert\">
        <i class=\"fas fa-exclamation-circle mr-2\"></i>
        <span class=\"\">Vous avez entré un code invalide.</span>
          <span type=\"button\" class=\"btn-close\" data-bs-dismiss=\"alert\" aria-label=\"Close\">&times</span>
        </div>
        </div>";
      }
        ...
        <div class="col-md-6">
            <div class="card text-center" style="top: 80px">
                <div class="card-header" style="color : #b2c7dd; background-color : #ffffff">
                  <h2 class="text-center" style="color : #b2c7dd">Code De Confirmation</h2>
                </div>
                <div class="card-body">
                    <form action="" method="post" autocomplete="off">
                        ...
                        <div class="form-group d-flex justify-content-center mt-0">
                           <input class="form-control col-md-9" type="number" name="code" placeholder="Entrer le code" required>
                        </div>
                        <p class="text-muted" style="font-size: 12px">Veuillez confirmer votre adresse e-mail pour pouvoir utiliser votre compte</p>
                        <hr class="hr-ligne">
